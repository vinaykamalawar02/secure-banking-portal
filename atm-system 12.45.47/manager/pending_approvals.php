...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if ($user_id && in_array($action, ['approve', 'reject'])) {
            try {
                // Only allow manager to approve users assigned to them
                if ($action === 'approve') {
                    $stmt = $conn->prepare("UPDATE users SET status = 'active' WHERE id = ? AND status = 'pending' AND manager_id = ?");
                    $stmt->execute([$user_id, $_SESSION['user_id']]);
                    if ($stmt->rowCount() > 0) {
                        $success = 'User approved successfully!';
                        log_activity($_SESSION['user_id'], 'manager', 'Approved user ID: ' . $user_id);
                    } else {
                        $error = 'User not found or not assigned to you.';
                    }
                } else {
                    $stmt = $conn->prepare("UPDATE users SET status = 'inactive' WHERE id = ? AND status = 'pending' AND manager_id = ?");
                    $stmt->execute([$user_id, $_SESSION['user_id']]);
                    if ($stmt->rowCount() > 0) {
                        $success = 'User rejected successfully!';
                        log_activity($_SESSION['user_id'], 'manager', 'Rejected user ID: ' . $user_id);
                    } else {
                        $error = 'User not found or not assigned to you.';
                    }
                }
...
// Get pending users assigned to this manager
$stmt = $conn->prepare("
    SELECT * FROM users 
...
<body class="bg-gray-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            ...
            <p class="text-gray-600">Review and approve new user registrations for your branch</p>
        </div>
            ...
            <div class="p-6">
                <?php if (empty($pending_users)): ?>
                    <div class="text-center py-8">
                        ...
                        <p class="text-gray-400 text-sm">All user registrations for your branch have been processed</p>
                    </div>
                <?php else: ?>
                    <div class="space-y-4">
